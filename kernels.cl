...
kernel void propagate(global float* cells,
    ...
    local float* local_tot_u, int tt,
    global float* global_tot_u, 
    int tot_cells)
{
    ...
    ** scratch space grid */
    ...
    local_tot_u[local_ii + local_jj * BLOCK_SIZE] = (obstacles[ii + jj * nx]) ? 0 : (native_sqrt((u_x * u_x) + (u_y * u_y)) / tot_cells);
    barrier(CLK_LOCAL_MEM_FENCE);

    reduce(local_tot_u, global_tot_u, tt, tot_cells);
}
    ...
    int tt, int tot_cells) 
{
    ...
    if (local_ii == 0 && local_jj == 0) {
        ...
        atomic_float_add(&global_tot_u[tt], sum);
    }
}